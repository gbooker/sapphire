...
- (void)onlyEnableFirstAudioTrack:(Movie)mov
{
	int tkCount = GetMovieTrackCount(mov);
	int i;
	BOOL enabledFirstAudio = NO;
	for(i=0; i<tkCount; i++)
	{
		Track track = GetMovieIndTrack(mov, i);
		if(!GetTrackEnabled(track))
		...
		SoundDescriptionHandle soundDesc = (SoundDescriptionHandle) NewHandle(0);
		GetMediaSampleDescription(media, 1, (SampleDescriptionHandle)soundDesc);
		
		BOOL detectedAC3 = NO;
		if((*soundDesc)->dataFormat == 'ac-3')
			detectedAC3 = YES;
		...
		if(enabledFirstAudio)
			SetTrackEnabled(track, NO);
		else if(GetTrackEnabled(track))
			enabledFirstAudio = YES;
	}
}
...
- (void)checkIfCanEnable
{
	...
	/*Turn off multiple audio tracks*/
	[self onlyEnableFirstAudioTrack:myMovie];
}
...
-(BOOL)setState:(BRMediaPlayerState)playState error:(NSError **)error;
{
	...
	if(!enabledChecked && playState != kBRMediaPlayerStateStopped)
	{
		[self checkIfCanEnable];
		enabledChecked = YES;
	}
