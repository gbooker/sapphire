 ...
 */
static NSString *commaSeparatedStringFromCollection( const NSSet * const set )
{
	if( [set count] == 0 )
		return nil;
...
-(void) parseVideo
{
	...
	NSMutableString * const videoText = [NSMutableString stringWithFormat:@"MPEG-%d", vts_VideoCodingMode( videoAttrs ) ];
...
-(void) parseAudio
{
	...
	if( numAudio > 0 )
	{
		...
		for( idx = 0; idx < numAudio; ++idx )
		{
			...
			if( lang != nil )
			{
				NSMutableString * const audioText = [NSMutableString stringWithString:(NSString *)lang];
...
-(void) parseSubtitles
{
	...
	if( numSubpictures > 0 )
	{
		...
		for( idx = 0; idx < numSubpictures; ++idx )
		{
			...
			{
				NSString * const lang = languageFromEncodedChars( vts_CommonFirstLanguageCode ( subpictureAttrs[idx] ),
